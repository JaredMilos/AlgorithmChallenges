...
function stepThroughArray(array) {
    function nextStep(index) {
        if (index == array.length - 1) {
            console.log("I made it!")
            return true;
        } else if (array[index] == 0) {
            console.log("I am on index " + index + " and I can't jump. :(")
        }
        for (let i = 1; i <= array[index]; i++) {
            if (index + i < array.length) {
                console.log("I am on index " + index + " and I am jumping to index " + (index + i));
                if (nextStep(index + i) == true) {
                    return true;
                } 
            }
        }
